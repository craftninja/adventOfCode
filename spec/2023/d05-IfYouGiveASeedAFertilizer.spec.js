...
describe('ifYouGiveASeedAFertilizer', () => {
  describe('closestLocationOfSeedSite', () => {
    it(`returns 35 when given \n"${rawAlmanac}"`, () => {
      expect(ifYouGiveASeedAFertilizer.closestLocationOfSeedSite(rawAlmanac)).toBe(35);
    });
    it(`returns 35 when given \n"${rawAlmanac}\n"`, () => {
      expect(ifYouGiveASeedAFertilizer.closestLocationOfSeedSite(rawAlmanac + "\n")).toBe(35);
    });
  })

  describe('support functions', () => {
    describe('getMappedNumber', () => {
      it('can get a mapped number 5 from original number 5', () => {
        expect(
          ifYouGiveASeedAFertilizer.getMappedNumber(5, [
            { destination: 50, source: 98, rangeLength: 2},
            { destination: 52, source: 50, rangeLength: 48},
          ])
        ).toBe(5)
      })
      it('can get a mapped number 81 from original number 79', () => {
        expect(
          ifYouGiveASeedAFertilizer.getMappedNumber(79, [
            { destination: 50, source: 98, rangeLength: 2},
            { destination: 52, source: 50, rangeLength: 48},
          ])
        ).toBe(81)
    ...
    describe('processAlmanac', () => {
      it('can create almanac object from raw almanac data', () => {
        expect(
          ifYouGiveASeedAFertilizer.processAlmanac(rawAlmanac)
        ).toEqual({
          seeds: [79, 14, 55, 13],
          seedToSoilMap: [
            { destination: 50, source: 98, rangeLength: 2},
    ...
    describe('sumArray', () => {
      it('can sum an array', () => {
        expect(ifYouGiveASeedAFertilizer.sumArrayRemovingNaNs([1,2,3])).toBe(6)
      })
    })
  })
});
