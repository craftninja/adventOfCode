const pipeMaze = {
  ...
  nestSize: (rawMaze) => {
    const loopCoordinates = pipeMaze.generateLoop(rawMaze)
    const areaOfLoop = pipeMaze.shoestringArea(loopCoordinates)
    return pipeMaze.picksInteriorCoordinates(
      areaOfLoop,
      loopCoordinates
    )
  },
  ...
  shoestringArea: (loopCoordinates) => {
    let area = 0;

    for (let i = 0; i < loopCoordinates.length; i++) {
      const j = (i + 1) % loopCoordinates.length;
      area += loopCoordinates[i][0] * loopCoordinates[j][1];
      area -= loopCoordinates[j][0] * loopCoordinates[i][1];
    }

    return Math.abs(area) / 2;
  },
  picksInteriorCoordinates: (area, coordinates) => {
    return area + 1 - (coordinates.length / 2)
  },
}
